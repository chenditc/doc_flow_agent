...
"""
    ...
    def _extract_python_code_from_response(self, response: Any) -> str:
        """Extract Python code from an LLM response.

        Contract:
        - Response must contain a tool call named ``generate_python_code``.
        - The tool call arguments must include a ``code`` string.
        """

        if not isinstance(response, dict):
            raise ValueError("LLM response must be an object with tool_calls")
