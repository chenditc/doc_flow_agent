...
async def test_create_and_complete_job(manager):
    ...
    finished = await manager.wait_for(job.job_id, timeout=1.0)
...
async def test_multiple_jobs_concurrency(manager):
    ...
    await asyncio.gather(
        manager.wait_for(job1.job_id, timeout=1.0),
        manager.wait_for(job2.job_id, timeout=1.0),
    )
...
async def test_cancel_job(manager, monkeypatch):
    ...
    await manager.wait_for(job.job_id, timeout=1.0)
